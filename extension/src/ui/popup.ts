...
const logsEl = document.querySelector("#logs") as HTMLUListElement;
...
document.querySelector("#start")?.addEventListener("click", async () => {
  await chrome.runtime.sendMessage({ type: "RUN_TOOL", payload: { id: crypto.randomUUID(), tool: "system.ping", args: {}, safety: { sessionId: "local", readOnly: true, approvalRequired: false, allowDomains: [], maxActions: 1, maxPages: 1 }, client: { name: "extension-ui", version: "0.1.0", clientId: "popup" } } });
  log("Bridge started");
  await refreshStatus();
});
